    ...
    <view class="container">
      ...
      <scroll-view scroll-y class="category-aside">
        <view class="cate-list">
          <view v-for="(item,index) in cateLeft" :key="item.cat_id" @tap="handleTapCate(index)" :class="currentIndex == index ? 'active': '' ">{{item.cat_name}}</view>
          <view class="redBar" :style="{top:(currentIndex * 100 + 30) +'rpx'}"></view>
        </view>
      </scroll-view>
      <!-- right -->
  ...
  methods: {
      ...
      renderContentPage(index){
        uni.request({
                 ...
                 const cateAll = res[1].data.message;
                 //把数据保存到本地，减少请求时间
                 //保存时间戳
                 ...
                 */
                 this.cateRight = [];
                 setTimeout(()=>{this.cateRight = cateAll[index].children},0);
